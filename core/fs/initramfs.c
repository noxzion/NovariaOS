...
void initramfs_load(multiboot_info_t* mb_info) {
    ...
    initramfs_load_from_memory(initramfs_data, initramfs_size);
}

void initramfs_load_from_memory(void* data, size_t size) {
    if (data == NULL || size == 0) {
        kprint("No initramfs data to load\n", 14);
        return;
    }
    
    uint8_t *initramfs_data = (uint8_t*)data;
    size_t initramfs_size = size;
