...
#define MAGIC_ALLOC      0xABCD123456789ABC
#define MAGIC_FREE       0xDCBA567890ABCDEF
#define ALIGNMENT        8
...
static uint64_t calculateChecksum(MemoryBlock* block);
...
static uint64_t calculateChecksum(MemoryBlock* block) {
    ...
    for (size_t i = 0; i < sizeof(MemoryBlock) - sizeof(uint64_t); i++) {
        sum = (sum << 3) ^ bytes[i];
    }
    return sum;
}
